...
// 				"user_id" : user_id,
	...
	function getCommentList(){
		...
		$.get(url, function(rData){
			...
			$.each(rData,function(){
				...
				// 삭제버튼 찾기
					tds.find(".btnCommentDelete").attr("data-recno", this.recno);
				// 수정 버튼
					tds.find(".btnCommentModify").attr("data-recno", this.recno);
	...
	$("#table_comment_list").on("click", ".btnCommentModify", function(){
		...
		console.log(tr);
		var tr_content = tr.find("td").eq(1);
		var re_comment = tr.find("td").eq(1).text();
		console.log(tr_content);
		console.log(re_comment);
		$(tr_content).html("<input type='text' class='contentModify' value='" + re_comment +"'/>");
		
// 		$(this).fadeOut("slow");
// 		$(this).next("#btnModifyRunComment").fadeIn("slow");
		$(".btnModifyRunComment").attr("data-recno", $(this).attr("data-recno"));
	});
	
	// 댓글 수정 저장 버튼
	$("#table_comment_list").on("click", ".btnModifyRunComment", function() {
		console.log("댓글 수정완료 버튼 누름");
		var re_comment = $(this).parents("tr").find(".contentModify").val();
		var recno = $(this).attr("data-recno");
				...
				<table style="display: none;" id="table_clone">
					<tr>
						...
						<td>
							...
							<button type="button" class="btn btn-sm btn-outline-success btnModifyRunComment" 
